...
Start from [cloud-layer-shader.md](references/cloud-layer-shader.md):
...
For another pipeline or engine, port the fragment logic as described in that file's porting notes.
...
Tune against [tuning.md](references/tuning.md) at the player's default camera. Change one parameter group per pass, capture each pass, and name what the capture showed before choosing the next change. Copy the kept values back into the shader property defaults.

**Done when:** the kept pass is chosen from captures, not from numbers alone, and the user's art direction (stylized by default in that file) is visible: crisp edges, a lit rim with a shade core, no confetti specks, no shade that reads as holes.

## 4. Prove coverage and motion

- Run [scripts/cloud_coverage.py](scripts/cloud_coverage.py) with the final values over every look-down angle that shows sky and at least 12 yaws. It must report 0 holes.
